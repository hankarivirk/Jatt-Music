...
class Inline:
    ...
    def controls(
        ...
        kb = []
        if status:
            kb.append([self.ikb(text=status, callback_data=f"controls status {chat_id}")])
        elif timer:
            kb.append([self.ikb(text=timer, callback_data=f"controls status {chat_id}")])
        if not remove:
            kb.append([
    ...
    def help_markup(self, _lang: dict, back: bool = False) -> types.InlineKeyboardMarkup:
        ...
        btns = [self.ikb(_lang[f"help_{i}"], callback_data=f"help {cb}") for i, cb in enumerate(cbs)]
