...
GENRES = {
    ...
    "random":    "top music hits worldwide",
    "rap":       "best rap hip hop songs",
    "rock":      "best rock songs",
    "pop":       "best pop songs",
    "jazz":      "best jazz music",
    "edm":       "best edm electronic songs",
    "rnb":       "best rnb soul songs",
    "country":   "best country songs",
    "bhojpuri":  "best bhojpuri songs hits",
    "tamil":     "best tamil songs hits",
    "telugu":    "best telugu songs hits",
    "kannada":   "best kannada songs",
    "malayalam": "best malayalam songs",
    "marathi":   "best marathi songs",
    "bengali":   "best bengali songs",
    "urdu":      "best urdu songs",
    "persian":   "best persian songs",
    "thai":      "best thai songs hits",
    "indonesian":"best indonesian songs",
    "filipino":  "best opm songs hits",
    "french":    "best french songs",
    "german":    "best german songs",
    "italian":   "best italian songs",
    "lofi":      "best lofi chill songs",
    "classical": "best classical music",
    "bollywood": "top bollywood hits songs",
    "party":     "best party anthems songs",
}
...
def _is_correct(answer: str, title: str) -> bool:
    ...
    if t_words:
        matched = sum(
            1 for w in t_words
            if w in a_words or any(_similarity(w, aw) >= 0.8 for aw in a_words if len(aw) > 2)
        )
...
def _file_valid(path: str) -> bool:
    try:
        return os.path.isfile(path) and os.path.getsize(path) > _MIN_FILE_KB * 1024
    except Exception:
        return False


def _score_bar(scores: dict, max_show: int = 5) -> str:
    if not scores:
        return ""
    top = sorted(scores.items(), key=lambda x: x[1], reverse=True)[:max_show]
    lines = []
    for i, (uid, pts) in enumerate(top):
        medal = MEDALS[i] if i < 3 else f"  {i + 1}."
        lines.append(f"  {medal}  <code>{uid}</code>  —  <b>{pts}</b> pts")
    return "\n".join(lines)
...
async def _end_game(chat_id: int):
    ...
    if not scores:
        lines.append("  ◈  No points scored this game.")
...
async def _round_timeout(chat_id: int):
    ...
    if not g or not g["active"] or g.get("answered"):
        return
    g["streak"] = {}
    ...
    try:
        await app.send_message(
            chat_id,
            f"⏱  <b>Time's up!</b>  No one guessed.\n\n"
            f"  ◈  The answer was  —  <b>{title}</b>"
        )
    except Exception:
        pass
...
async def _hint_timer(chat_id: int):
    ...
    if artist:
        try:
            await app.send_message(
                chat_id,
                f"🎤  <b>Artist hint</b>  —  <code>{artist}</code>"
            )
        except Exception:
            pass
...
async def _next_round(chat_id: int):
    ...
    if not track.file_path or not _file_valid(track.file_path):
        try:
            await msg.edit_text("⚠️  Could not load track. Skipping...")
        except Exception:
            pass
    ...
    try:
        ...
        try:
            await msg.edit_text("⚠️  VC error. Skipping round...")
        except Exception:
            pass
        await asyncio.sleep(2)
        asyncio.create_task(_next_round(chat_id))
...
async def _startgame(_, m: types.Message):
    ...
    for a in m.command[1:]:
        if a.isdigit():
            rounds = max(3, min(25, int(a)))
        elif len(a) >= 2:
            genre = a.lower()

    rounds = min(rounds, len(GENRES) + 10)
    ...
    for r in data:
        try:
            ...
            if not dur or dur < 45 or dur > 600:
                continue
            pool.append(Track(
                id=r["id"],
...
async def _score(_, m: types.Message):
    ...
    if not g:
        return await m.reply_text("◈  No active game right now.")
    scores = sorted(g["scores"].items(), key=lambda x: x[1], reverse=True)
    if not scores:
        return await m.reply_text("◈  No scores yet — guess the song!")
...
async def _gameskip(_, m: types.Message):
    ...
    if g.get("answered"):
        return await m.reply_text("◈  Round already answered — next one coming up.")
    await _cancel_tasks(g)
    g["answered"] = True
    g["streak"] = {}
...
async def _cb_lb(_, q: types.CallbackQuery):
    ...
    if not rows:
        return await q.answer("No leaderboard yet.", show_alert=True)
...
async def _answer_listener(_, m: types.Message):
    if not m.from_user or m.from_user.is_bot or not m.text:
        return
    g = _games.get(m.chat.id)
    if not g or not g["active"] or g.get("answered") or not g.get("current"):
        return
    title = g["current"].title or ""
    if not title or not _is_correct(m.text, title):
        return
